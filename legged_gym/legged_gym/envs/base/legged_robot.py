...
from phc.utils.motion_lib_smpl_xue import MotionLibSMPLXUE
...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.init_done = True
        self.trajectories = torch.zeros(self.num_envs, (self.num_dof * 3 + 3 + 3) * 100).to(self.device) # 19dof + 19dofvel + 3angular velocity + 3projectedgravity + 19lastaction
        self.trajectories_with_linvel = torch.zeros(self.num_envs, (self.num_dof * 3 + 3 + 3 + 3) * 100).to(self.device) # 19dof + 19dofvel + 3angular velocity + 3projectedgravity + 19lastaction
        ...
        if self.cfg.motion.teleop:
            if self.cfg.asset.name == "g1":
                self.extend_body_parent_ids = [17, 21]
            else:
            ...
            if self.cfg.motion.extend_head:
                ...
                if self.cfg.asset.name == "g1":
                    self.extend_body_pos = torch.tensor([[0.3, 0, 0], [0.3, 0, 0], [0, 0, 0.4]]).repeat(self.num_envs, 1, 1).to(self.device)
                else:
    ...
    def step(self, actions):
        ...
        num = self.num_dof * 3 + 3 + 3

        self.trajectories[:, 1 * num :] = self.trajectories[:, :-1 * num].clone()
        self.trajectories[:, 0 * num : 1 * num] = current_obs_a.clone()

        lin_vel = self.base_lin_vel
        current_obs_a_with_linvel = torch.cat((dof, dof_vel, lin_vel, base_ang_vel, base_gravity, actions), dim=1)
        self.trajectories_with_linvel[:, 1 * (num+3) :] = self.trajectories_with_linvel[:, :-1 * (num+3)].clone()
        self.trajectories_with_linvel[:, 0 * (num+3) : 1 * (num+3)] = current_obs_a_with_linvel.clone()
        if self.cfg.train_velocity_estimation:
    ...
    def post_physics_step(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(self._rigid_body_rot[:, self.pelvis_idx, :], self._rigid_body_ang_vel[:, self.pelvis_idx, :])
        self.projected_gravity[:] = quat_rotate_inverse(self._rigid_body_rot[:, self.pelvis_idx, :], self.gravity_vec)
        ...
        if self.cfg.motion.teleop:
            if self.cfg.asset.terminate_by_ref_motion_distance:
                ...
                if self.cfg.asset.local_upper_reward:
                    ...
                    ref_body_pos[:, self.pelvis_idx:] -= diff
            ...
            if self.cfg.asset.terminate_by_1time_motion:
                time = (self.episode_length_buf) * self.dt + self.motion_start_times 
                if self.cfg.motion.play:
                    self.time_out_by_1time_motion = False
                else:
    ...
    def compute_self_and_task_obs(self, ):
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.asset.local_upper_reward:
                ...
                ref_body_pos_extend[:, self.pelvis_idx:] -= diff
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == 'v1':
                ...
                if self.cfg.env.add_short_history:
                    ...
                    history_to_be_append = self.trajectories[:, 0:self.cfg.env.short_history_length*(self.ndof * 3 + 3 + 3)]
                    obs = torch.cat([dof_pos, dof_vel, base_ang_vel, base_gravity,  # 19dim + 19dim + 3dim + 3dim 
                                                task_obs,  # 
                                                self.actions,
                ...
                if self.cfg.env.add_short_history:
                    assert self.cfg.env.short_history_length > 0
                    num = self.num_dof * 3 + 3 + 3
                    history_to_be_append = self.trajectories[:, 0:self.cfg.env.short_history_length*num]
                    obs = torch.cat([dof_pos, dof_vel, base_ang_vel, base_gravity,  # 19dim + 19dim + 3dim + 3dim 
                                                task_obs,  # 
                                                self.actions,
                                                history_to_be_append], dim = -1) # 19dim
    ...
    def _init_buffers(self):
        ...
        if self.cfg.motion.teleop:
            ...
            self._load_motion()
            if self.cfg.motion.play:
                self.marker_coords = torch.zeros(self.num_envs, 25 * self.cfg.motion.num_traj_samples, 3, dtype=torch.float, device=self.device, requires_grad=False) # extend
            else:
    ...
    def _load_motion(self):
        ...
        if not self.cfg.motion.play:
            ...
            self._motion_lib = MotionLibSMPLXUE(device=self.device, masterfoot_conifg=None, multi_thread=False) #multi_thread=True doesn't work
            sk_tree = SkeletonTree.from_mjcf("dep/phc/phc/data/assets/mjcf/smpl_humanoid_1.xml")
    ...
    def _reward_knee_distance(self):
        ...
        left_knee_xy = self._get_rigid_body_pos("left_knee_link")[..., :2]
        right_knee_xy = self._get_rigid_body_pos("right_knee_link")[..., :2]
        knee_distance_xy = torch.norm(left_knee_xy - right_knee_xy, dim=-1)
        return (knee_distance_xy < 0.06)* 1.0
    ...
    def _reward_stable_lower_when_vrclose(self):
        ...
        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_vr = diff_global_body_pos[:, -3:-1] # left hand, right hand
    ...
    def _reward_teleop_body_position_feet(self):
        feet = self._rigid_body_pos[:, self.feet_indices]
        offset = self.env_origins + self.env_origins_init_3Doffset
        motion_times = (self.episode_length_buf ) * self.dt + self.motion_start_times # next frames so +1

        motion_res = self._get_state_from_motionlib_cache_trimesh(self.motion_ids, motion_times, offset= offset)
        ref_dof_pos = motion_res['rg_pos_t'][:, self.feet_indices]

        feet_diff = feet - ref_dof_pos
        feet_dist = (feet_diff**2).mean(dim=-1).mean(dim=-1)
        r_feet = torch.exp(-feet_dist / 0.03)
        return r_feet
    ...
    def _reward_lower_action_rate(self):
        # Penalize changes in actions
        return torch.sum(torch.square(self.last_actions[:, :self.pelvis_idx] - self.actions[:, :self.pelvis_idx]), dim=1)
    ...
    def _reward_freeze_upper_body(self):
        return torch.mean(torch.square(self.dof_pos[:, self.pelvis_idx-1:] - self.default_dof_pos[:, self.pelvis_idx-1:]), dim=1)
    ...
    def _reward_teleop_joint_position_lower(self):
        ...
        diff_dof_pos = diff_dof_pos[:, :self.pelvis_idx-1] # lower, not including torso
    ...
    def _reward_teleop_joint_position_upper(self):
        ...
        diff_dof_pos = ref_dof_pos - dof_pos
        diff_dof_pos = diff_dof_pos[:, self.pelvis_idx-1:] # upper
    ...
    def _reward_teleop_joint_vel_lower(self):
        ...
        diff_dof_vel = diff_dof_vel[:, :self.pelvis_idx-1] # lower, not including torso
    ...
    def _reward_teleop_joint_vel_upper(self):
        ...
        diff_dof_vel = ref_dof_vel - dof_vel
        diff_dof_vel = diff_dof_vel[:, self.pelvis_idx-1:] # upper
        diff_dof_vel_dist = torch.mean(torch.square(diff_dof_vel), dim=1)
        r_dof_vel = torch.exp(-diff_dof_vel_dist / self.cfg.rewards.teleop_joint_vel_sigma)
        return r_dof_vel
    ...
    def _reward_teleop_body_position_extend(self):
        ...
        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_lower = diff_global_body_pos[:, :self.pelvis_idx]
        diff_global_body_pos_upper = diff_global_body_pos[:, self.pelvis_idx:]
    ...
    def _reward_teleop_body_rotation_lower(self):
        ...
        diff_global_body_rot = torch_utils.quat_mul(ref_body_rot, torch_utils.quat_conjugate(body_rot))
        diff_global_body_angle = torch_utils.quat_to_angle_axis(diff_global_body_rot)[0]
        diff_global_body_angle = diff_global_body_angle[:, self.pelvis_idx:] # lower
        diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
        r_body_rot = torch.exp(-diff_global_body_angle_dist / self.cfg.rewards.teleop_body_rot_sigma)
        return r_body_rot
    ...
    def _reward_teleop_body_rotation_upper(self):
        ...
        diff_global_body_rot = torch_utils.quat_mul(ref_body_rot, torch_utils.quat_conjugate(body_rot))
        diff_global_body_angle = torch_utils.quat_to_angle_axis(diff_global_body_rot)[0]
        diff_global_body_angle = diff_global_body_angle[:, self.pelvis_idx:] # upper
        diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
        r_body_rot = torch.exp(-diff_global_body_angle_dist / self.cfg.rewards.teleop_body_rot_sigma)
        return r_body_rot
    ...
    def _reward_teleop_body_vel_lower(self):
        ...
        diff_global_vel = ref_body_vel - body_vel
        diff_global_vel = diff_global_vel[:, self.pelvis_idx:] # lower
        diff_global_vel_dist = (diff_global_vel**2).mean(dim=-1).mean(dim=-1)
    ...
    def _reward_teleop_body_vel_upper(self):
        ...
        diff_global_vel = ref_body_vel - body_vel
        diff_global_vel = diff_global_vel[:, self.pelvis_idx:] # upper
        diff_global_vel_dist = (diff_global_vel**2).mean(dim=-1).mean(dim=-1)
    ...
    def _reward_teleop_body_ang_vel_lower(self):
        ...
        diff_global_ang_vel = ref_body_ang_vel - body_ang_vel
        diff_global_ang_vel = diff_global_ang_vel[:, self.pelvis_idx:] # lower
        diff_global_ang_vel_dist = (diff_global_ang_vel**2).mean(dim=-1).mean(dim=-1)
        r_ang_vel = torch.exp(-diff_global_ang_vel_dist / self.cfg.rewards.teleop_body_ang_vel_sigma)
        return r_ang_vel
    ...
    def _reward_teleop_body_ang_vel_upper(self):
        ...
        diff_global_ang_vel = ref_body_ang_vel - body_ang_vel
        diff_global_ang_vel = diff_global_ang_vel[:, self.pelvis_idx:] # upper
        diff_global_ang_vel_dist = (diff_global_ang_vel**2).mean(dim=-1).mean(dim=-1)
        r_ang_vel = torch.exp(-diff_global_ang_vel_dist / self.cfg.rewards.teleop_body_ang_vel_sigma)
        return r_ang_vel
...
def compute_imitation_observations_teleop_max(root_pos, root_rot, body_pos,   ref_body_pos, ref_body_vel, time_steps,  ref_episodic_offset = None, ref_vel_in_task_obs = True):
    ...
    heading_inv_rot = torch_utils.calc_heading_quat_inv(root_rot)
    heading_rot = torch_utils.calc_heading_quat(root_rot)
    # 将逆矩阵和旋转矩阵扩展到与body_pos相同的形状
    heading_inv_rot_expand = heading_inv_rot.unsqueeze(-2).repeat((1, body_pos.shape[1], 1)).repeat_interleave(time_steps, 0)
    heading_rot_expand = heading_rot.unsqueeze(-2).repeat((1, body_pos.shape[1], 1)).repeat_interleave(time_steps, 0)
    
    ##### Body position and rotation differences
    # 计算body_pos和ref_body_pos之间的差异
    diff_global_body_pos = ref_body_pos.view(B, time_steps, J, 3) - body_pos.view(B, 1, J, 3)
    # 将差异转换为局部坐标
    diff_local_body_pos_flat = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), diff_global_body_pos.view(-1, 3)) # 

    ##### body pos + Dof_pos This part will have proper futuers.
    # 计算ref_body_pos相对于根节点的局部坐标
    local_ref_body_pos = ref_body_pos.view(B, time_steps, J, 3) - root_pos.view(B, 1, 1, 3)  # preserves the body position
    local_ref_body_pos = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), local_ref_body_pos.view(-1, 3))
    
    # 计算ref_body_vel相对于根节点的局部坐标
    local_ref_body_vel = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), ref_body_vel.view(-1, 3))

    # 如果ref_episodic_offset不为None，则进行偏移计算
